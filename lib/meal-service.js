...
class MealService {
  ...
  postFoodsToMeal(mealId, mealName) {
    ...
    for (var i = 0; i < checkedFoods.length; i++) {
      ...
      fetch(`${this.baseUrl}/${mealId}/foods/${$food.data('id')}`, this.postFoodToMealConfig())
        .then(handleResponse)
  ...
  deleteFoodFromMeal(e) {
    ...
    let mealName = e.target.parentNode.parentNode.parentNode.parentNode.id
    let foodId = e.target.parentNode.parentNode.dataset.id
    fetch(`${this.baseUrl}/${mealId}/foods/${foodId}`, {method: "DELETE"})
      .then(this.removeFoodRow(e))
      .then(this.deleteFoodFromMealObject(mealName, foodId))
      .then(this.updateMealCalAfterDelete(mealName))
      .then(this.appendTotalsTable())
      .catch(errorLog)
  }
  ...
  appendFoodToMeal($food, mealName) {
    let foodId = $food.data('id')
    let foodName = $food.find('[data-id=name]').html()
    let foodCalories = $food.find('[data-id=calories]').html()
